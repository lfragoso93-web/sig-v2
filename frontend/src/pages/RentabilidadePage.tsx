  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex flex-col">
          ...
          <span className="text-[10px]" style={{ color: 'var(--color-text-faint)' }}>
            {classe.asset_count} ativo{classe.asset_count !== 1 ? 's' : ''}
          </span>
        </div>
        {classe.twr_available && accumulatedTwr != null ? (
          <div className="flex flex-col items-end">
            <span className="text-xs tabular-nums font-semibold" style={{ color: pnlColor(accumulatedTwr) }}>
              {pnlSign(accumulatedTwr)}{formatPercent(accumulatedTwr)}
            </span>
            <span className="text-[10px]" style={{ color: 'var(--color-text-faint)' }}>
              TWR acumulado{classe.return_is_estimated ? ' estimado' : ''}
            </span>
          </div>
        ) : (
          <span className="text-[10px] text-right max-w-[145px]" style={{ color: 'var(--color-text-faint)' }}>
            {classe.performance_status === 'awaiting_backfill'
              ? 'Histórico aguardando materialização'
              : classe.performance_reason ?? 'TWR indisponível'}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between gap-3 mb-2">
        ...
        <span className="text-xs tabular-nums" style={{ color: pnlColor(capitalResult) }}>
          {capitalResult == null ? '—' : `${pnlSign(capitalResult)}${formatBRL(capitalResult)}`}
        </span>
      </div>
      ...
      <div className="w-full rounded-full" style={{ height: 7, background: 'var(--color-surface-dynamic)' }}>
        <div
          ...
          style={{ width: `${Math.min(allocation, 100)}%`, background: 'var(--color-primary)' }}
        />
      </div>
      ...
      {classe.has_partial_prices && (
        <p className="text-[10px] mt-1" style={{ color: 'var(--color-warning)' }}>
          Cobertura parcial de preços no fechamento da classe
        </p>
      )}
    </div>
  )
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">TWR fechado, resultados financeiros e benchmarks</p>
        </div>
        {kpis?.snapshot_date && (
          <span className="text-xs" style={{ color: 'var(--color-text-faint)' }}>
            Performance até {new Date(kpis.snapshot_date + 'T00:00:00').toLocaleDateString('pt-BR')}
            {kpis.return_is_estimated ? ' · estimada' : ''}
          </span>
        )}
      </div>
      ...
      {loadingKpis ? (
        ...
        <>
          <div className="kpi-grid">
            <KpiCard label="Patrimônio atual" value={formatBRL(safeNum(kpis?.patrimonio_atual))} subValue={formatBRL(safeNum(kpis?.custo_total))} subLabel="Custo das posições abertas" />
            <KpiCard label="Resultado total" value={formatBRL(safeNum(kpis?.total_pnl))} valueColor={pnlColor(kpis?.total_pnl)} />
            <KpiCard label="TWR no mês" value={kpis ? `${pnlSign(kpis.retorno_mes_pct)}${formatPercent(safeNum(kpis.retorno_mes_pct))}` : '—'} valueColor={pnlColor(kpis?.retorno_mes_pct)} />
            <KpiCard label="TWR em 12 meses" value={kpis ? `${pnlSign(kpis.retorno_12m_pct)}${formatPercent(safeNum(kpis.retorno_12m_pct))}` : '—'} valueColor={pnlColor(kpis?.retorno_12m_pct)} />
          </div>

          <div className="kpi-grid">
            <KpiCard label="TWR desde o início" value={kpis ? `${pnlSign(kpis.retorno_desde_inicio_pct)}${formatPercent(safeNum(kpis.retorno_desde_inicio_pct))}` : '—'} valueColor={pnlColor(kpis?.retorno_desde_inicio_pct)} subValue={kpis ? `${pnlSign(kpis.retorno_dia_pct)}${formatPercent(safeNum(kpis.retorno_dia_pct))}` : '—'} subLabel="Último fechamento" />
            <KpiCard label="Resultado realizado" value={formatBRL(safeNum(kpis?.ganho_realizado))} valueColor={pnlColor(kpis?.ganho_realizado)} />
            ...
            <KpiCard label="Resultado não realizado" value={formatBRL(safeNum(kpis?.ganho_nao_realizado))} valueColor={pnlColor(kpis?.ganho_nao_realizado)} />
          </div>
        </>
      )}

      {kpis?.has_partial_prices && (
        <p className="text-xs" style={{ color: 'var(--color-warning)' }}>
          A performance fechada possui cobertura parcial de preços e deve ser interpretada como estimada.
        </p>
      )}
      ...
      <div className="flex flex-col gap-4">
        <div className="card overflow-hidden">
          ...
          <div className="p-4">
            {loadingClasses ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">{[...Array(3)].map((_, i) => <div key={i} className="h-24 skeleton rounded" />)}</div>
            ) : (classes ?? []).length === 0 ? (
        ...
        <div className="card overflow-hidden">
          <div className="section-card-header flex-wrap gap-2">
            <div className="flex flex-col">
              <span className="text-xs font-semibold">Resultado por ativo</span>
              <span className="text-[10px]" style={{ color: 'var(--color-text-faint)' }}>Percentuais simples de resultado; não representam TWR individual</span>
            </div>
              ...
              <table className="w-full min-w-[920px]">
                <thead>
                  <tr style={{ borderBottom: '1px solid oklch(from var(--color-text) l c h / 0.08)' }}>
                    {['Ativo', 'Qtd', 'P.M.', 'Val. atual', 'Resultado n.r.', 'Resultado real.', 'Resultado total'].map(h => (
                      <th key={h} className={`pb-2 text-xs font-medium ${h === 'Ativo' ? 'text-left' : 'text-right'} ${h !== 'Resultado total' ? 'pr-3' : ''}`} style={{ color: 'var(--color-text-faint)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
