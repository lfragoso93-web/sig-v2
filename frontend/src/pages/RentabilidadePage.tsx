  ...
  return (
    <div className="rounded-xl p-3" style={{ background: 'var(--color-surface-offset)', border: '1px solid var(--color-divider)' }}>
      <div className="flex items-center justify-between gap-3 mb-2">
        ...
        <span className="text-xs tabular-nums" style={{ color: pnlColor(pnlPct) }}>
      ...
      <div className="text-[10px] tabular-nums text-right mt-1" style={{ color: 'var(--color-text-faint)' }}>
      ...
      {loadingKpis ? (
        ...
        <>
          <div className="kpi-grid">
            <KpiCard label="Patrimônio atual" value={formatBRL(safeNum(kpis?.patrimonio_atual))} subValue={formatBRL(safeNum(kpis?.total_aportado))} subLabel="Total aportado" />
            <KpiCard label="Retorno total" value={formatBRL(safeNum(kpis?.total_pnl))} change={safeNum(kpis?.retorno_total_pct)} />
            <KpiCard label="Retorno no mês" value={kpis ? `${pnlSign(kpis.retorno_mes_pct)}${formatPercent(safeNum(kpis.retorno_mes_pct))}` : '—'} change={safeNum(kpis?.retorno_mes_pct)} />
            <KpiCard label="Retorno 12 meses" value={kpis ? `${pnlSign(kpis.retorno_12m_pct)}${formatPercent(safeNum(kpis.retorno_12m_pct))}` : '—'} change={safeNum(kpis?.retorno_12m_pct)} />
          </div>

          <div className="kpi-grid">
            <KpiCard label="Ganho não realizado" value={formatBRL(safeNum(kpis?.ganho_nao_realizado))} subValue={kpis ? `${pnlSign(kpis.retorno_desde_inicio_pct)}${formatPercent(safeNum(kpis.retorno_desde_inicio_pct))}` : '—'} subLabel="Desde o início" />
            <KpiCard label="Ganho realizado" value={formatBRL(safeNum(kpis?.ganho_realizado))} />
            ...
            <KpiCard label="Custo médio total" value={formatBRL(safeNum(kpis?.custo_total))} />
          </div>
        </>
      )}
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="p-4">
            {loadingClasses ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {[...Array(3)].map((_, i) => <div key={i} className="h-20 skeleton rounded" />)}
              </div>
            ) : (classes ?? []).length === 0 ? (
              <p className="text-xs" style={{ color: 'var(--color-text-faint)' }}>Sem dados</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {(classes ?? []).map(c => <ClasseBar key={c.asset_type} classe={c} />)}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="card overflow-hidden">
          <div className="section-card-header flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <select value={assetTypeFilter} onChange={e => setAssetTypeFilter(e.target.value)} className="input text-xs" style={{ width: 220 }}>
                {ASSET_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
              <button onClick={() => setShowZeradas(v => !v)} className="px-3 py-1 rounded text-xs font-medium transition-colors" style={{ background: showZeradas ? 'oklch(from var(--color-primary) l c h / 0.15)' : 'var(--color-surface-offset)', color: showZeradas ? 'var(--color-primary)' : 'var(--color-text-muted)' }}>
                Posições zeradas
              </button>
            </div>
          </div>
