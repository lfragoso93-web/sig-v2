...
// Labels amigáveis usadas como fallback se o backend não enviar asset_label
  ...
  return (
    <div className="w-full overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {groups.map(group => {
            // Resolve o label do grupo: prioriza o que vem do backend
            const groupLabel = group.label ?? ASSET_LABELS[group.asset_type] ?? group.asset_type
            return (
              <>
                {/* Linha de grupo */}
                <tr
                ...
                >
                  <td colSpan={4} className="px-4 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-slate-300">{groupLabel}</span>
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        {/* badge usa asset_label amigável, não o enum bruto */}
                        <span className={`asset-badge ${assetBadgeClass(p.asset_type)}`}>
                          {p.asset_label ?? ASSET_LABELS[p.asset_type] ?? p.asset_type}
                        </span>
                      </div>
                    </td>
