...
// Estilos de celulas usando variaveis CSS — responsivos ao tema claro/escuro
...
function AssetMenu({ ticker, assetLabel, assetType }: AssetMenuProps) {
  ...
  const items = [
    ...
    },
    {
      ...
      onClick: () => {
        setOpen(false)
        // Rota correta dentro do layout protegido
          ...
          <tr style={{ borderBottom: '1px solid var(--color-divider)' }}>
            ...
            <th className="text-right px-4 py-2 font-medium" style={cellMuted}>
              <span className="flex items-center justify-end gap-1">
                P. Atual
                <HelpCircle size={10} style={cellFaint} title="Cotação via BRAPI/yfinance. '—' quando indisponível." />
              </span>
            </th>
          ...
          {groups.map(group => (
            <>
              ...
              {expanded[group.label] && (group.positions ?? []).map(item => {
                ...
                // cores de resultado
                const varColor = item.variation_value >= 0
                  ? 'var(--color-success)'
                  : 'var(--color-error)'
