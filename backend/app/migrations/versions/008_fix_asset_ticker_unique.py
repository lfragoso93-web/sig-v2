...
para tipos de ativo distintos e desbloqueando o cache L1 de cotações.
...
def upgrade():
    # Remove o índice único simples de ticker (se existir)
    with op.batch_alter_table('assets') as batch_op:
        try:
            batch_op.drop_index('ix_assets_ticker')
        except Exception:
            pass  # índice pode ter nome diferente dependendo do ambiente

        # Cria índice não-único em ticker (para buscas por ticker apenas)
