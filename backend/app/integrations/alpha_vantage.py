...
Limites do plano free: 25 req/min, 500 req/dia.
Com 4 ativos internacionais atualizando a cada 30min em 12h de mercado:
  - Cotacao: 4 x 24 = ~96 req/dia
  - Historico: 4 req/dia (compact diario)
  - Total: ~100 req/dia — bem abaixo do limite.

Rate limiter dedicado: alpha_vantage_limiter (4 req/min, burst 4).
Importado e usado por quotes_service e price_history_service.
"""
...
async def fetch_global_quote(ticker: str) -> Optional[float]:
    ...
    try:
        ...
        if not quote:
            # Pode ser nota de limite de rate ou ticker invalido
            note = data.get("Note") or data.get("Information") or ""
            if note:
                logger.warning("[alpha_vantage] GLOBAL_QUOTE %s: %s", ticker, note[:120])
            else:
        ...
        logger.warning("[alpha_vantage] fetch_global_quote %s erro: %s", ticker, e)
        return None
...
async def fetch_daily_history(
    ...
    Selecao automatica de outputsize:
      - days_back <= 100  → compact (100 registros, 1 request pequeno)
      - days_back > 100   → full   (~20 anos, usado apenas na primeira importacao)

    Retorna lista de (datetime UTC, close_price) ordenada do mais antigo ao mais recente.
    Retorna [] se a API key nao estiver configurada ou em caso de erro.
    """
    if not _is_configured():
        logger.debug("[alpha_vantage] ALPHA_VANTAGE_API_KEY nao configurada — pulando")
        return []
    ...
    try:
        ...
        note = data.get("Note") or data.get("Information") or ""
        if note:
            logger.warning("[alpha_vantage] TIME_SERIES_DAILY %s: %s", ticker, note[:120])
            return []
        ...
        for date_str, ohlc in series.items():
            try:
                dt = datetime(
                    ...
                    int(date_str[5:7]),
                    int(date_str[8:10]),
                    21, 0, 0,          # 21:00 UTC = fechamento NY (17:00 ET)
                    tzinfo=timezone.utc,
                )
                if dt < cutoff:
                    continue
        ...
        logger.info(
            "[alpha_vantage] TIME_SERIES_DAILY %s: %d registros (outputsize=%s)",
            ticker, len(rows), output_size,
        )
        return rows

    except Exception as e:
        logger.warning("[alpha_vantage] fetch_daily_history %s erro: %s", ticker, e)
        return []
