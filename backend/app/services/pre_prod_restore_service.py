...
def restore_postgres_backup(
    *,
    ...
    if backup_report.get("schema_version") != "pre-prod-backup.v3":
        raise BackupError(
            "backup incompatível; gere novamente com pre-prod-backup.v3"
        )
    if backup_report.get("consistent_snapshot") is not True:
        raise BackupError(
            "backup não comprova snapshot consistente entre inventário e dump"
        )
