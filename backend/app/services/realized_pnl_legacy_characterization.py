"""Caracterização legada de PnL para listas isoladas de transações.

Este módulo não é uma fronteira de runtime. Ele existe temporariamente para testes
históricos e consumidores em memória que não possuem acesso ao catálogo global de
eventos corporativos. Novos fluxos devem usar ``realized_pnl_projection_reader``.
"""
...
def calculate_realized_pnl_by_ticker(
    ...
    """Caracteriza a regra histórica sem consultar o catálogo corporativo."""
...
def calculate_realized_pnl(transactions: list[Transaction]) -> float:
    """Soma a caracterização histórica por ticker."""
