...
def _calc_realized_from_txs(
    txs: list,
    actions_by_ticker: dict[str, tuple[EligibleQuantityAction, ...]] | None = None,
) -> dict[str, float]:
    qty_map: dict[str, float] = {}
    cost_map: dict[str, float] = {}
    realized: dict[str, float] = {}
    action_cursors: dict[str, int] = {}
    actions_by_ticker = actions_by_ticker or {}
    ...
    for tx in txs:
        ...
        if ticker not in qty_map:
            ...
            cost_map[ticker] = 0.0
            realized[ticker] = 0.0
        projected, cursor, _ = consume_quantity_actions(
            actions_by_ticker.get(ticker, ()),
            cursor=action_cursors.get(ticker, 0),
            through_date=tx.date,
            quantity=Decimal(str(qty_map.get(ticker, 0.0))),
        )
        qty_map[ticker] = float(projected)
        ...
        ).upper() == "USD" or normalize_type(asset_type_raw) in _USD_ASSET_TYPES
...
async def _get_realized_pnl_by_ticker(
    ...
    return _calc_realized_from_txs(txs, actions)
...
async def _calc_invested_up_to(
    db: AsyncSession, portfolio_id: int, up_to: date
) -> float:
    result = await db.execute(
        select(Transaction)
        .where(
    ...
    for tx in txs:
        ...
        if is_usd:
            ...
            if saved is not None and float(saved or 0) > 0:
                fx_rate = float(saved)
        value = qty * price * fx_rate + (
            fees * fx_rate if op == OperationType.buy else 0
        )
...
async def _positions_enriched_without_rf(
    db: AsyncSession, portfolio_id: int
) -> list[dict]:
    ...
    price_items = [
        {"ticker": p["ticker"], "asset_type": p["asset_type"]} for p in positions_raw
    ]
    try:
...
async def _kpis_from_realtime(db: AsyncSession, portfolio_id: int) -> dict:
    ...
    try:
        ...
        custo_inicio_mes = await _calc_invested_up_to(
            db, portfolio_id, ultimo_dia_mes_anterior
        )
...
def _ret_between(
    snap_end: PortfolioSnapshot, snap_start: Optional[PortfolioSnapshot]
) -> float:
...
async def get_kpis(db: AsyncSession, portfolio_id: int) -> dict:
    ...
    snap_mes = await _snapshot_at(
        db, portfolio_id, today.replace(day=1) - timedelta(days=1)
    )
    ...
    payload = {
        ...
        "total_pnl": round(
            float((snap_today.unrealized_pnl or 0) + (snap_today.realized_pnl or 0)), 2
        ),
        ...
        "retorno_desde_inicio_pct": _ret_between(
            snap_today, snap_first if snap_first != snap_today else None
        ),
        "proventos_total": round(proventos_total, 2),
        "proventos_12m": round(proventos_12m, 2),
        "snapshot_date": snap_today.snapshot_date.isoformat()
...
async def get_rentabilidade_por_ativo(
    ...
    actions = await load_eligible_quantity_actions(
        db,
        tickers=[str(tx.ticker) for tx in flat_txs],
        through_date=date.today(),
    )
    realized_map = _calc_realized_from_txs(flat_txs, actions)
    enriched = await _positions_enriched_without_rf(db, portfolio_id)
    ...
    for pos in enriched:
        ...
        unrealized_pnl = current_value - total_invested
        unrealized_pct = (
            _safe_pct(Decimal(str(unrealized_pnl)), Decimal(str(total_invested)))
            if total_invested
            else 0.0
        )
...
async def get_rentabilidade_por_classe(
    ...
    for at, v in agg.items():
        ...
        total_pct = (
            _safe_pct(Decimal(str(total_pnl)), Decimal(str(v["total_invested"])))
            if v["total_invested"]
            else 0.0
        )
        allocation_pct = (
