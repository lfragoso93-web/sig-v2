"""Executa comparação read-only de Day Trade canônico e legado.
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compara a projeção quantitativa canônica de Day Trade com o motor "
...
def comparison_to_dict(comparison: DayTradeAnnualComparison) -> dict[str, object]:
    monthly = []
    ...
    divergent_months = sum(1 for item in comparison.monthly if not item.is_match)
    return {
        "schema_version": "irpf-day-trade-canonical-legacy-comparison.v1",
...
async def _main(arguments: argparse.Namespace) -> int:
    ...
    async with AsyncSessionLocal() as db:
        comparison = await compare_annual_day_trade_with_legacy(
            db,
...
def main() -> None:
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": "irpf-day-trade-comparison-error.v1",
                    "ok": False,
                    "error": str(exc),
                },
                ensure_ascii=False,
