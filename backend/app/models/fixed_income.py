...
class FixedIncomeInvestment(Base, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(String(200), nullable=False)  # ex: "CDB Banco XYZ 110% CDI"
    institution: Mapped[str] = mapped_column(String(150), nullable=False)  # ex: "Banco XYZ"
    ...
    rate: Mapped[Decimal] = mapped_column(Numeric(10, 4), nullable=False)  # ex: 110.0 para 110% CDI
    invested_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), nullable=False)
    date_start: Mapped[date] = mapped_column(Date, nullable=False)
    date_maturity: Mapped[date | None] = mapped_column(Date, nullable=True)  # None = sem vencimento
    ...
    def __repr__(self) -> str:
        return f"<FixedIncome id={self.id} name={self.name} indexer={self.indexer}>"
