...
def test_synthetic_twr_availability_contract_keeps_fixed_income_unavailable() -> None:
    assert _EXPECTED["RENDA_FIXA"] == (
        False,
...
def test_twr_availability_gate_does_not_materialize_or_call_provider() -> None:
    ...
    assert "rf_daily_twr=false" in source
    assert "issue149=open" in source
    for forbidden in (
        "AsyncSession",
        "db.",
        "provider",
