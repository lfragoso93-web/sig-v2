...
@pytest.mark.asyncio
async def test_get_portfolio_performance_single_stock():
    db = AsyncMock(spec=AsyncSession)
    ...
    with patch('app.services.performance_service.cache_get') as mock_cache_get, \
        ...
        assert result["total_invested"] == 5010.0
        assert result["total_current_value"] == 6000.0
        assert result["total_gain"] == 990.0
...
async def test_get_portfolio_performance_multiple_stocks():
    ...
    with patch('app.services.performance_service.cache_get') as mock_cache_get, \
        ...
        asset_result = MagicMock()
        asset_result.scalar_one_or_none.side_effect = [mock_asset1, mock_asset2]
        
        db.execute.side_effect = [tx_result, asset_result, asset_result]
