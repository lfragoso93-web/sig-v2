...
@pytest.mark.asyncio
async def test_partial_cdi_uses_latest_covered_rate_before_target(monkeypatch) -> None:
    coverage = AsyncMock(
        side_effect=[
            BenchmarkCoverageStatus.PARTIAL,
            BenchmarkCoverageStatus.COMPLETE,
        ]
    )
    latest_covered = AsyncMock(return_value=date(2026, 9, 8))
