...
def test_other_runtime_services_do_not_import_legacy_characterization() -> None:
    violations: list[str] = []

    for path in sorted(_SERVICES.glob("*.py")):
        if path.name in {
            "realized_pnl_service.py",
            "realized_pnl_legacy_characterization.py",
        }:
            continue
    ...
    assert violations == [], (
        "o calculador histórico só pode ser reexportado pela fronteira pública "
        f"temporária: {violations}"
    )
