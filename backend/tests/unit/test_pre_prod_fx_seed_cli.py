...
from app.cli import pre_prod_fx_seed as cli
from app.services.pre_prod_fx_seed_service import FxSeedAlreadyRunningError

RUN_ID = "20260727-083000"
BRANCH = "stable-15jun"
COMMIT_SHA = "a" * 40
...
async def test_cli_returns_zero_and_prints_contract_on_success(monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, "run_pre_prod_fx_seed", runner)

    exit_code = await cli._main()

    assert exit_code == cli.EXIT_OK
    payload = json.loads(capsys.readouterr().out)
    assert payload["schema_version"] == "pre-prod-fx-seed.v1"
...
async def test_cli_returns_operational_failure_for_invalid_result(monkeypatch, capsys):
    result = SimpleNamespace(
        ok=False,
        to_dict=lambda: {"ok": False, "errors": ["integridade"]},
    )
    monkeypatch.setattr(cli, "run_pre_prod_fx_seed", AsyncMock(return_value=result))
...
async def test_cli_returns_distinct_code_for_concurrent_execution(monkeypatch, capsys):
    monkeypatch.setattr(
        ...
        AsyncMock(side_effect=FxSeedAlreadyRunningError("estágio cambial já está em execução")),
    )
    ...
    payload = json.loads(capsys.readouterr().out)
    assert payload == {"ok": False, "error": "estágio cambial já está em execução"}
...
async def test_cli_redacts_unexpected_exception_message(monkeypatch, capsys):
    ...
    assert payload["error"] == "falha inesperada no estágio cambial"
