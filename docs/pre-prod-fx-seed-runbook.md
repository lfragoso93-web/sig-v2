...
2. `HEAD` aprovado e registrado na Issue #217.
...
$SecondOutput = docker compose exec `
    -e "PRE_PROD_BRANCH=stable-15jun" `
    -e "PRE_PROD_COMMIT_SHA=$CommitSha" `
    backend python -m app.cli.pre_prod_fx_seed `
        --run-id $SecondRunId `
...
Interromper e registrar na Issue #217 quando ocorrer qualquer um dos seguintes:
